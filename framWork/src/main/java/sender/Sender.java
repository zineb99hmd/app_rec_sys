...
public class Sender {
    ...
    public static void offlineStrategy(String fileItem, String fileEvent, List<Algorithm> AlgoList, int recsLimit, float trainingRatio, List<Metric> metricsList) {
        ...
        for (Algorithm alg:AlgoList) {
            ...
            for (Item t: items) {
                alg.handleItemUpdate(t);
            }

            for (Event e : trainingSet) {
                alg.handleEventNotification(e);
            }
            ...
            for (Long idUser : listUsers) {
                ...
                alg.getRecommendations(request);

                userTransactions = transactionsForAllUsersToEvaluate.get(idUser);
                for (Metric metric : metricsList) {

                    metric.evaluate((long) 0.0, alg.getRecommendations(request), userTransactions);
                }
    ...
    public static void onlineStrategy(String fileItem, String fileEvent,  List<Algorithm> AlgoList,int recsLimit, long evaluationWindowSize,List<Metric> metricsList) {
        {
            for (Algorithm alg : AlgoList) {
                List<Notification> trainingWindow = ReadPlista.loadPlistaData(fileItem, fileEvent);
...
//                    page3HorsLigne.progress.setProgress(i);
                    ...
                    if (itemOrEvent instanceof Item) {
                        ...
                        alg.handleEventNotification(currentTransaction);
                        ...
                        if (!userTransactions.isEmpty()) {
                            ...
                            for (Metric metric : metricsList) {
                                metric.evaluate(request.getTime().getTime(), alg.getRecommendations(request), userTransactions);
                ...
                for (int i = 0; i < alg.getRecommendations(request).size(); i++) {
                    RecList.add(String.valueOf((alg.getRecommendations(request)).get(i)));
    ...
    public static void onlineStrategyWithIa(String fileItem, String fileEvent,  List<Algorithm> AlgoList,int recsLimit,List<Metric> metricsList) {
        {
